...
M0–M6 已关闭。本机工作台、独立启动器、库文件备份、本仓库广场 API 与独立管理台可用。已验证平台是 macOS，未验证 Windows / Linux。无商店包、无公开下载声明。M7 提案已接受，正在补齐已登录收藏、令牌轮换与 `GET /v1/admin/me`。M8 设置对齐（原型十类）已写入规格与计划；M7 关闭前不改设置应用代码，且不得删除已有本机设置。
...
- 产品名：提示方舟 / PromptArk
- 阶段：M6 运营后台（完成）；M7 合同补齐（提案已接受，实现中）；M8 设置对齐已立项
- 版本：未发行；无商店包、无公开下载声明
