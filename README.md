...
M0–M5 已关闭。本机工作台、独立启动器、库文件备份与本仓库广场 API 可用。已验证平台是 macOS，未验证 Windows / Linux。无商店包、无公开下载声明。M6 合同已接受（ADR 0009）。尚未创建 `admin-web`。
...
- 产品名：提示方舟 / PromptArk
- 阶段：M5 在线广场（完成）；M6 进行中（Task 2 审核 API）
- 版本：未发行；无商店包、无公开下载声明
