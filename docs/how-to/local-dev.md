...
M0–M5 已关闭。浏览器可预览工作台；桌面窗口才能写 SQLite、登记全局快捷键、备份库文件、持久化 Refresh。
...
3. 点「提示词广场」：API 未开时出现「当前离线」和「前往本地」；本地库仍可用。启动器只搜本地。API 开着时，点卡片「下载」不弹登录，切回本地能看到该条；点「收藏」登录原因含「收藏」，本地不因此多出一条。登录后「发布提示词」未选源时提交禁用；提交后仍能改本地正文。
