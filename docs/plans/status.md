...
|---|---|
| 状态 | 现行（活文档，只写今天为真的事） |
| 更新日期 | 2026-08-24（M8 已关闭；无变量预览与 Enter 已映射） |
...
1. M8 已关闭。没有 M9。variables「无变量」「Enter 前进」已映射到 `UsePromptModal.spec.js`。下一个本机切片要用户点名（Windows/Linux 核验、商店包、OAuth、云同步等不得假装接通）。
