...
describe("desktopPrefs", () => {
  ...
  it("persists launch at login and tray on linux", async () => {
    const saved = {};
    await saveDesktopPref(DESKTOP_PREF_KEYS.launchAtLogin, true, "linux", async (key, value) => {
      saved[key] = value;
    });
    await saveDesktopPref(DESKTOP_PREF_KEYS.minimizeToTray, true, "linux", async (key, value) => {
      saved[key] = value;
    });
    expect(saved.launch_at_login).toBe("1");
    expect(saved.minimize_to_tray).toBe("1");
